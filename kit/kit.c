...
int main(int argc, char ** argv) {
    ...
    int bufsize = BUFFSIZE; // allow buffer to be changed
    ...
    // if no infile is specified, also read it from stdin
    int no_infiles = (optind == argc);

    for(int i = optind; i < argc || no_infiles; i++) {
        // if argv = "-", get the input from stdin
        is_stdin = no_infiles || (strcmp(argv[i], "-") == 0);
